...
const apiData_1 = require("../utils/apiData");
const criteriaMap = {
    1: "facilities",
    2: "price",
    3: "distance",
...
function convertToFuzzyWeights(weights) {
    const fuzzyWeights = {};
...
function calculateFuzzyWeightedSum(places, weights) {
    // Step 1
    ...
    return places.map(place => {
        const rawScore = place.place_scores.reduce((total, score) => {
...
async function getRecommendations(req, res) {
    ...
    try {
        const cityData = await (0, apiData_1.getDataFromAPI)(city.toString());
